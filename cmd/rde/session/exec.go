...
connection, nohup it inside the session instead.`,
  ...
  bitrise-cli rde session exec SESSION_ID --output json -- ls -la /opt`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return renderExecResult(cmd, format, res, shellHint(args[1:], shellMode, res.ExitCode))
		},
	}
...
// into a non-zero local exit in BOTH output modes. In --output json the
...
func renderExecResult(cmd *cobra.Command, format output.Format, res internalrde.ExecResult, hint string) error {
	if format == output.JSON {
		if err := output.Render(cmd.OutOrStdout(), format, res, nil); err != nil {
			return err
		}
...
// "Error: …" line is suppressed — the remote command's stderr (or the JSON
// envelope) already explains the failure — and the message is written here
// so it never lands in stdout next to a JSON envelope.
