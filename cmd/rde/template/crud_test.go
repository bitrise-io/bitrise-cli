...
func TestCreateCmd_DeviceFlags(t *testing.T) {
	...
	c := newCreateCmd()
	c.SetIn(strings.NewReader(`{"name":"iOS","stack_id":"osx-xcode-16.0.x-edge","machine_type":"g2.mac","device_spec":{"platform":"android"}}`))
	stdout, _, err := run(t, c, srv.URL, "ws-1",
		[]string{"--file", "-", "--device-platform", "ios", "--device-model", "iPhone 16", "--device-os-version", "18.2"}, output.Human)
	if err != nil {
		t.Fatalf("Execute: %v", err)
	}
...
func TestCreateCmd_DeviceSpecFromFile(t *testing.T) {
	...
	c := newCreateCmd()
	c.SetIn(strings.NewReader(`{"name":"Android","stack_id":"ubuntu-android","machine_type":"standard","device_spec":{"platform":"android","system_image":"system-images;android-34;google_apis;x86_64"}}`))
	if _, _, err := run(t, c, srv.URL, "ws-1", []string{"--file", "-"}, output.Human); err != nil {
		t.Fatalf("Execute: %v", err)
	}
	spec, _ := gotBody["deviceSpec"].(map[string]any)
...
func TestCreateCmd_DeviceFlagsValidation(t *testing.T) {
	...
	for name, args := range map[string][]string{
		...
		c := newCreateCmd()
		c.SetIn(strings.NewReader(`{"name":"Dev","stack_id":"s","machine_type":"m"}`))
		if _, _, err := run(t, c, srv.URL, "ws-1", args, output.Human); err == nil || !strings.Contains(err.Error(), "--device-") {
...
func TestUpdateCmd_DeviceFlags(t *testing.T) {
	...
	_, _, err := run(t, newUpdateCmd(), srv.URL, "ws-1",
		[]string{uuidTemplateID, "--device-platform", "android", "--device-model", "pixel_7"}, output.Human)
	if err != nil {
		t.Fatalf("Execute: %v", err)
	}
	spec, _ := gotBody["deviceSpec"].(map[string]any)
...
func TestUpdateCmd_ClearDevice(t *testing.T) {
	...
	_, _, err := run(t, newUpdateCmd(), srv.URL, "ws-1", []string{uuidTemplateID, "--clear-device"}, output.Human)
	if err != nil {
		t.Fatalf("Execute: %v", err)
	}
	if gotBody["updateDeviceSpec"] != true {
	...
	gotBody = nil
	_, _, err = run(t, newUpdateCmd(), srv.URL, "ws-1",
		[]string{uuidTemplateID, "--clear-device", "--device-platform", "ios"}, output.Human)
...
func TestUpdateCmd_FileWithoutDeviceLeavesItAlone(t *testing.T) {
	...
	c := newUpdateCmd()
	c.SetIn(strings.NewReader(`{"name":"Renamed"}`))
	if _, _, err := run(t, c, srv.URL, "ws-1", []string{uuidTemplateID, "--file", "-"}, output.Human); err != nil {
		t.Fatalf("Execute: %v", err)
	}
