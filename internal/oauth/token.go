...
// tokenResponse covers the JSON returned by both the WorkOS token endpoint
// (code→JWT and refresh) and the monolith OIDC exchange (JWT→PAT). Fields not
// present in a given response stay zero.
...
// exchangeCodeForJWT trades an authorization code for a JWT + refresh token at
// the WorkOS token endpoint (grant_type=authorization_code). redirectURI must
// match the one sent on the authorize request.
...
// refreshJWT obtains a fresh JWT (and possibly a rotated refresh token) from
// the WorkOS token endpoint using a refresh token.
...
// jwtExpiry decides when a freshly obtained JWT expires: prefer the response's
// expires_in, fall back to the JWT's own exp claim, and finally to a short
// conservative window so a refresh is attempted soon rather than never.
